...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 4) When user picks from the dropdown → open chat
  chatSelect.addEventListener('change', async () => {
    const contactEmail = chatSelect.value;
    const box          = document.getElementById('chat-messages');
    box.innerHTML      = '';
    document.getElementById('reply-preview')?.remove();

    if (!contactEmail) {
      console.log("⚪️ Chat placeholder selected, unsubscribing.");
      unsubscribeChat?.();
      return;
    }

    console.log("▶️ Opening chat with:", contactEmail);
  ...
  document.getElementById('post-announcement')
    ?.addEventListener('click', async () => {
      ...
      try {
        ...
        console.log("📣 Announcement posted:", txt);
  ...
  auth.onAuthStateChanged(async user => {
    ...
    console.log("👤 Logged in as:", currentUser.email, isAgent ? "(Agent)" : "");
  ...
  // 8) Fallback Send-Button for dropdown chat
    ...
    ?.addEventListener('click', async () => {
      ...
      await db.collection('users').doc(leaderUid)
        ...
        .collection('messages').add({
          text:      txt,
          sender:    currentUser.email,       // unified field
          toEmail:   email,
          timestamp: firebase.firestore.FieldValue.serverTimestamp(),
